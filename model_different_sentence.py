...
class MyDataset(Dataset):
    def __init__(self, path, is_unlabeled=False, is_test=False, is_dev=False,  labeled_ratios = False):
        self.path = path
...
def get_dataloader(task, labeled_ratios=0.1, bs=64):
    if task=="albert":
        cvat_labeled = MyDataset("./data/albert/emobank_albert.pkl", labeled_ratios=labeled_ratios)
        cvat_unlabeled = MyDataset("./data/albert/emobank_albert.pkl", is_unlabeled=True)
        cvat_test = MyDataset("./data/albert/emobank_albert.pkl", is_test=True)
        cvat_dev = MyDataset("./data/albert/emobank_albert.pkl", is_dev=True)
    ...
    if task=="disbert":
        fb_labeled = MyDataset("./data/disbert/emobank_disbert.pkl", labeled_ratios=labeled_ratios)
        fb_unlabeled = MyDataset("./data/disbert/emobank_disbert.pkl", is_unlabeled=True)
        fb_test = MyDataset("./data/disbert/emobank_disbert.pkl", is_test=True)
        fb_dev = MyDataset("./data/disbert/emobank_disbert.pkl", is_dev=True)
    ...
    if task=="robert":
        emobank_labeled = MyDataset("./data/robert/emobank_robert.pkl", labeled_ratios=labeled_ratios)
        emobank_unlabeled = MyDataset("./data/robert/emobank_robert.pkl", is_unlabeled=True)
        emobank_test = MyDataset("./data/robert/emobank_robert.pkl", is_test=True)
        emobank_dev = MyDataset("./data/robert/emobank_robert.pkl", is_dev=True)
    ...
    if task=="ernie":
        emobank_labeled = MyDataset("./data/ernie/emobank_ernie.pkl", labeled_ratios=labeled_ratios)
        emobank_unlabeled = MyDataset("./data/ernie/emobank_ernie.pkl", is_unlabeled=True)
        emobank_test = MyDataset("./data/ernie/emobank_ernie.pkl", is_test=True)
        emobank_dev = MyDataset("./data/ernie/emobank_ernie.pkl", is_dev=True)
        emobank_labeled_loader = DataLoader(dataset=emobank_labeled,batch_size=bs,shuffle=True,collate_fn = collate_fn, num_workers=0)
        emobank_unlabeled_loader = DataLoader(dataset=emobank_unlabeled,batch_size=bs,shuffle=True,collate_fn = collate_fn,num_workers=0)
        emobank_test_loader = DataLoader(dataset=emobank_test,batch_size=bs,shuffle=True,collate_fn = collate_fn,num_workers=0)
        emobank_dev_loader = DataLoader(dataset=emobank_dev,batch_size=bs,shuffle=True,collate_fn = collate_fn,num_workers=0)
...
def train_bert(epochs=1200, task="albert", labeled_ratios=0.1, bs=8, exp_name="no_name"):
    ...
    labeled_loader, unlabeled_loader, test_loader, dev_loader =  get_dataloader(task, labeled_ratios, bs=bs)
    ...
    for epoch in range(epochs):
        for x, y in labeled_loader:
            ...
            pred = net(x.cuda())
            loss_labeled = (loss_fun(pred, y.cuda()))
            opt.zero_grad()
            loss.backward()
            opt.step()
...
def train(epochs=3000, task="albert", labeled_ratios=0.1, submodel=BiLSTM(), bs=8, noise=0.05, exp_name="no_name"):
...
if __name__ == '__main__':
    _, task, labeled_ratios, seed, submodel, epochs, bs, noise, exp_name = argv
    ...
    if submodel=="transformer":
        train(epochs=epochs, task=task, labeled_ratios=labeled_ratios, submodel=BatchFormer(), bs=bs, noise=noise, exp_name=exp_name)
    if submodel == "none":
