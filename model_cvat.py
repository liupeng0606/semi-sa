...
def get_best(net, dev_dataloader, test_loader, epoch):
...
def get_dataloader(task, labeled_ratios=0.1, bs=64):
    if task=="cvat":
        ...
        cvat_unlabeled = MyDataset("./data/processed_cvat.pkl", is_unlabeled=True)
    ...
    if task=="fb":
        ...
        fb_unlabeled = MyDataset("./data/fb.pkl", is_unlabeled=True)
    ...
    if task=="emobank":
        ...
        emobank_unlabeled = MyDataset("./data/emobank.pkl", is_unlabeled=True)
...
def train_bert(epochs=1200, task="cvat", labeled_ratios=0.1, bs=64, exp_name="no_name"):
    ...
    for epoch in range(epochs):
        ...
        count,epoch = get_best(net, dev_loader, test_loader, epoch)

        if count > 10:
            for key in all_list:
                print(all_list[key], " epoch:"+str(epoch))
                print()
            exit()



def train(epochs=1200, task="cvat", labeled_ratios=0.1, submodel=BiLSTM(), bs=64, noise=0.05, exp_name="no_name"):
